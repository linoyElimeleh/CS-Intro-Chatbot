        ...
        for (int i = romanNumber.length() - 1; i >= 0; i--) {
            switch (romanNumber.charAt(i)) {
                ...
                case 'D': 
                    ...
                    break;
                case 'M': 
                    nextLetterValue = 1000; 
                    break;
            }
            // If the next letter is less than the current letter, subtract the next letter value from the answer
            if (4 * nextLetterValue < ans) {
                ans -= nextLetterValue;
            } else {
                ans += nextLetterValue;
            }
        }
        System.out.println("The integer value of the roman number " + romanNumber + " is " + ans);
    }
}
